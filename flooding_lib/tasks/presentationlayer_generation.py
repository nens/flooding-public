...
#* Purpose    : convert a sobekmodel and resultfiles (his-files)
#*              to a Presentation_shape
...
def get_or_create_geo_presentation_source(
    scenario, pt, check_timestamp_original_sourcefile):
    '''
        create PresentationSource objects for given Flooding.Scenario and
        PresentationType
        input:
            scenario: Flooding.Scenario object
            pt: PresentationType object
            check_timestamp_original_sourcefile: check if shapefile is still
            up to date
        output:
            .....
    ...
    if pt.generation_geo_source == 'scenario_model':
        ...
        model_location = os.path.join(
            dest_dir, resultloc.replace('simulatie_rapport.zip', 'model.zip'))
        ...
        if scenario.sobekmodel_inundation:
            ...
            model_srid = 28992  # ssume this is a dutch model
        ...
        model_location = os.path.join(
            source_dir, project_fileloc, str(model.model_case))
    ...
    for part in pt.generation_geo_source_part.split(','):
        if part in ['nodes', 'branches']:
            #if generate nodes, branches is almost always also needed.
            #So create both at the same time to reduce creation time
    ...
    for part in generate_parts:
        if generate_parts[part] == True:
            if source_new[part] or files_updated:
                ...
                source[part].file_location = shapefiles[part].replace(
                    presentation_dir + '\\', '')
                source[part].save()
...
def get_or_create_presentationlayer(scenario, presentationtype):
    ...
    if not result.exists():
        ...
        result_value = result.first().value
    ...
    for pt in presentation_types:
        if pt.geo_type == PresentationType.GEO_TYPE_GRID:
            ...
            pass
                             ...
                             PresentationType.GEO_TYPE_POINT]:
            log.debug('type shape')
            get_or_create_shape_layer(scenario, pt, True)
        elif pt.geo_type == PresentationType.GEO_TYPE_NO_GEOM:
            log.debug('type no geom')
            log.debug('this is not for this task')
            pass
