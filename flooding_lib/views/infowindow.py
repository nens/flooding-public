...
def infowindow(request):
    ...
    if action_name == 'information':  # GET
        return infowindow_information(request, scenario)

    elif action_name == 'remarks':  # POST AND GET difference handled
                                    # in 'return method'
        callbackfunction = data.get('callback')
        ...
        with_approvalobject = bool(
            data.get('with_approvalobject', True))

        return infowindow_approval(
            request,
            data, scenario, callbackfunction,
...
def archive_scenario(request, permission_manager, scenario_id):
    ...
    return render(
        request, template, {'form': form, 'succeeded': succeeded})


def infowindow_information(request, scenario):
    return render(
        request,
...
def infowindow_remarks(
    ...
    return render(
        request,
        'flooding/infowindow_remarks.html',
        {'form': form,
...
def infowindow_approval(
        request, permission_manager, data, scenario, callbackfunction,
    form_id, with_approvalobject):
...
def showattachments(request, scenario_id):
    ...
    used_scenario = get_object_or_404(Scenario, pk=scenario_id)
    request_related_to = data.get('relatedto')
...
def editproperties(request, permission_manager, scenario_id):
    ...
    return render(
        request,
        'flooding/edit_scenario_properties.html',
        {'scenario': used_scenario,
         'form': form,
         'succeeded': succeeded})
